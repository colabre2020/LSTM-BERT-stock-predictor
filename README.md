...
1.	BERT Model – This model is trained on the financial news data to predict the sentiment around the stocks, the get the best prediction during the training and testing phase have to fine tune iteratively to reach an expected outcome like predicting the best sentiment around the stock based on the financial news data.
2.	LSTM Model – This model is trained on the S&P 50 stock data (Fig-4) with a 42 day window. The model performed well after multiple fine tuning of the hyper parameters. The model performance 
